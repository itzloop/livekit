...
func (m *SubscriptionManager) subscribe(sub *mediaTrackSubscription) error {
	...
	if m.params.StreamRoom {
		logger.Infow("in a stream room, only allow participants to sub to streamer")
		var (
		...
		logger.Debugw(fmt.Sprintf("%s is subbing to a track published by %s", p, publisher))
		// if p is not streamer and the track is not published by the streamer, avoid subbing
		if p != streamer && publisher != streamer {

			logger.Debugw(fmt.Sprintf("%s can't sub to the track published by %s beacuse %s is not a streamer", p, publisher, publisher))
